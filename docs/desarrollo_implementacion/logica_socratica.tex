El comportamiento socrático del tutor no reside en el modelo de lenguaje en sí, sino en las instrucciones que lo condicionan. Como se adelantó en el análisis comparativo (Sección~\ref{SS:EA_IA_COMPARATIVA}), SocratiCode no requiere reentrenar ningún modelo: toda la lógica pedagógica se inyecta en tiempo de ejecución a través de un \textit{system prompt} cuidadosamente diseñado y del contexto que se construye en cada petición.
...
Las reglas más determinantes son la \textbf{1}, la \textbf{3} y la \textbf{5}. La regla 1 evita la ruptura del contexto implícito: al recibir el código del editor de forma invisible, el tutor nunca tiene que interrumpir el diálogo socrático para pedir que el alumno pegue nada. La regla 3 cierra la vía más directa de fuga de información: prohíbe explícitamente mostrar código corregido, incluso en formato de ejemplo, y le proporciona al modelo una alternativa concreta (\textit{qué preguntar en lugar de qué mostrar}). La regla 5 protege el rol frente a ataques de inyección de instrucciones (\textit{prompt injection}) por parte del alumno.
